...
exports.updateMe = catchAsync(async (req, res, next) => {
  ...
  if (req.body.password || req.body.passwordConfirm) {
    return next(
      new AppError(
        'This URL is not for password updates. Please go to /updateMyPassword',
        400
      )
    )
  }
...
exports.getUserEditPrefill = catchAsync(async (req, res, next) => {
  const doc = await User.findById(req.params.id).populate(
    'installmentPlan plotInformation'
  )

  if (!doc) {
    return next(
      new AppError(`Could not find all data for the ID: ${req.params.id}`, 404)
    )
  }
